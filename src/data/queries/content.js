...
async function resolveFileName(path, locale) {
  ...
  maybeFileName = await resolveExtension(path, locale, '.md');
  if (maybeFileName.success) {
    return { success: true, fileName: maybeFileName.fileName, extension: '.md' };
  }

  maybeFileName = await resolveExtension(path, locale, '.html');
  if (maybeFileName.success) {
